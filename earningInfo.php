<DIV class="product-item float-clear" >


<div class=" col-sm-3" >
	<DIV class="col-sm-12 ">Name<span id="EIname-error" class="signup-error"></span></DIV>
	<input type="text" name="name1[]" id="EIname" placeholder="Father/Mother/brother/sister Earning member Name" class="form-control"/>
</div>
<div class=" col-sm-2">
		<DIV class="col-sm-12 ">Realtion</DIV>
	

	<DIV class="float-left"><select name="relation1[]" class="form-control">
  <option value="father">Father</option>
  <option value="mother">Mother</option>
  <option value="brother">Brother</option>
  <option value="sister">Sister</option>
</select>
</div>
</div>
<div class=" col-sm-3">
	<DIV class="col-sm-12 ">Occupation<span id="EIocc-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="ocp[]" id="EIocc" placeholder="Type Occupation"  class="form-control"/></DIV>
</div>
<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Organization<span id="EIorgan-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="org[]" id="EIorgan" placeholder="Type Organization if applicable" class="form-control"/></DIV>
</div>
<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Designation<span id="EIdes-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="dsg[]" id="EIdes" placeholder="Type Designation" class="form-control"/></DIV>
</div>
<div class=" col-sm-3">
	<DIV class="col-sm-12 ">Monthly Earning<span id="EIMonEarn-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="mearng[]" id="EIMonEarn" placeholder="Monthly Earning in Rupees" class="form-control"/></DIV>
</div>
<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Remarks<span id="EIRemarks-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="rmrks[]" id="EIRemarks"  class="form-control"/></DIV>
</div>

<div class=" col-sm-3">
	<DIV class="col-sm-12 ">Cnic No <span id="EIcnic-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="cnic[]" id="EIcnic" placeholder="CNIC  without dashes" class="form-control"/></DIV>
</div>
<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Status</DIV>


	<DIV class="float-left"><select name="status[]" id="EIStatus" class="form-control">
	<option value="Alive">Alive</option>
	<option value="Deceased">Deceased</option>

	</select>
	</div>
</div>

<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Prof. Status</DIV>


	<DIV class="float-left"><select name="prf_status[]" id="EIProSt" class="form-control">
	<option value="Employed">Employed</option>
	<option value="Retired">Retired</option>
	<option value="BusinessOwner">Business Owner</option>

	</select>
	</div>
</div>
<div class=" col-sm-3">
	<DIV class="col-sm-12 ">Mobile<span id="EImob-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="mobile[]" id="EImob" placeholder="Mobile no." class="form-control"/></DIV>

</div>
<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Office Number<span id="EIOffNo-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="office_num[]" id="EIOffNo" placeholder="Office telephone no." class="form-control"/></DIV>
</div>
<div class=" col-sm-3">
	<DIV class="col-sm-12 ">Company<span id="EIcomp-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="cmpny_name[]" id="EIcomp" placeholder="company name if applicable" class="form-control"/></DIV>
</div>
<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Grade<span id="EIgrade-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="grade[]" id="EIgrade" placeholder="grade" class="form-control"/></DIV>
</div>

<div class=" col-sm-2">
	<DIV class="col-sm-12 ">Ntn<span id="EINtn-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><input type="text" name="ntn[]" id="EINtn" class="form-control"/></DIV>
</div>

<div class=" col-sm-3">
	<DIV class="col-sm-12 ">Address<span id="EIAdd-error" class="signup-error"></span></DIV>
	<DIV class="float-left"><textarea name="address[]"  id="EIAdd" placeholder="Type address here" rows="4" cols="73" ></textarea></DIV>
	<!--<DIV class="float-left"><input type="text" name="address[]" id="EIAdd" class="form-control"/></DIV>-->

</div>

<DIV class="float-left"><input type="checkbox" name="item_index[]" class="form-control"/></DIV>




</DIV>